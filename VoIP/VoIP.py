...
class VoIPCall:
    ...
    def cancel(self) -> None:
        """Sendet einen CANCEL-Request für einen Anruf im DIALING-Zustand"""
        if self.state != CallState.DIALING:
            raise InvalidStateError("Call is not in dialing state")
        
        # CANCEL-Request erstellen
        message = self.sip.gen_cancel(self.request)
